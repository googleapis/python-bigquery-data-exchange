...
from google.cloud.bigquery.dataexchange.common import common_pb2  # type: ignore
from google.cloud.bigquery_data_exchange_v1beta1.services.analytics_hub_service import AnalyticsHubServiceAsyncClient
...
from google.cloud.bigquery_data_exchange_v1beta1.services.analytics_hub_service import transports
...
def test_list_data_exchanges_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, dataexchange.DataExchange)
                   for i in results)
...
def test_list_listings_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, dataexchange.Listing)
                   for i in results)
...
def test_get_listing(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = dataexchange.Listing(
            ...
            icon=b'icon_blob',
            categories=[common_pb2.Category.CATEGORY_OTHERS],
            request_access='request_access_value',
            bigquery_dataset=dataexchange.Listing.BigQueryDatasetSource(dataset='dataset_value'),
        )
    ...
    assert response.icon == b'icon_blob'
    assert response.categories == [common_pb2.Category.CATEGORY_OTHERS]
